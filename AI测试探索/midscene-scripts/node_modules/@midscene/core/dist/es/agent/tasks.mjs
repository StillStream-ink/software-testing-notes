...
import { multimodalPromptToChatMessages } from "../ai-model/shared/multimodal-prompt.mjs";
import { standardPlan } from "../ai-model/workflows/planning/index.mjs";
import { getReadableTimeString, userPromptToMultimodalPrompt, userPromptToString } from "../common.mjs";
...
import { withFileChooser } from "./file-chooser.mjs";
...
class TaskExecutor {
    ...
    async action(userPrompt, planningModel, defaultModel, includeLocateInPlanning, aiActContext, cacheable, replanningCycleLimitOverride, imagesIncludeCount, deepThink, fileChooserAccept, deepLocate, abortSignal, reportOptions) {
        return withFileChooser(this.interface, fileChooserAccept, async ()=>this.runAction(userPrompt, planningModel, defaultModel, includeLocateInPlanning, aiActContext, cacheable, replanningCycleLimitOverride, imagesIncludeCount, deepThink, deepLocate, abortSignal, reportOptions));
    }
        ...
        while(true){
            ...
            const result = await session.appendAndRun({
                ...
                executor: async (param, executorContext)=>{
                    ...
                    const planImpl = 'custom' === planningModel.adapter.planning.kind ? planningModel.adapter.planning.planFn : standardPlan;
                    let planResult;
                    try {
