...
class UIObservationImpl {
    ...
    ensureFixedWindowOptions(options) {
        assert(options?.domIncluded === void 0, 'UIObservation does not support domIncluded because it only evaluates recorded screenshots');
    }
...
class UIObserverImpl {
    ...
    async finalizeStop() {
        ...
        try {
            ...
            if (this.source && lastFrame?.persisted) {
                ...
                representative.screenshot = ScreenshotItem.fromFile(this.writer.resolveFramePath(lastFrame.persisted), lastFrame.persisted.mimeType, lastFrame.capturedAt);
    ...
    async finalizeRecord(endedAt) {
        assert(this.stopped && this.representative, 'observation must be stopped before finalizing the observed window');
    ...
    async persistUnstoredFrames() {
        ...
        for(let start = 0; start < uncachedFrames.length; start += DECODE_BATCH_SIZE){
            ...
            const decoded = this.source ? await this.source.decode(batch) : batch.map((frame)=>{
                assert(isImageDataUrl(frame.ref), 'fallback observation frame must be an image data URL');
                return frame.ref;
            });
    ...
    constructor(deps, opt){
        ...
        this.writer = deps.observationRecordWriter ?? new UIObservationRecordWriter();
    }
}
function uiContextFromObservationRecord(record) {
    assert('midscene_ui_observation' === record.type && 1 === record.version, 'invalid UI observation record type or version');
    assert(record.frames.length > 0, 'UI observation record contains no frames');
    assert(Number.isFinite(record.shotSize.width) && record.shotSize.width > 0, 'UI observation record shot width must be positive');
    assert(Number.isFinite(record.shotSize.height) && record.shotSize.height > 0, 'UI observation record shot height must be positive');
    assert(Number.isFinite(record.shrunkShotToLogicalRatio) && record.shrunkShotToLogicalRatio > 0, 'UI observation record screenshot ratio must be positive');
    const screenshotSequence = record.frames.map((frame)=>ScreenshotItem.fromFile(frame.path, frame.mimeType, frame.capturedAt));
    return {
...
export { UIObservationImpl, UIObserverImpl, uiContextFromObservationRecord };
