...
function headersToEntries(headers) {
    const entries = [];
    new Headers(headers).forEach((value, key)=>entries.push([
            key,
            value
        ]));
    return entries;
...
function wrapOpenAICompatibleFetch(context) {
    ...
    return async (input, init)=>{
        ...
        if (context.recordEvent) {
            ...
            context.recordEvent({
                ...
                request: {
                    ...
                    body: await request.clone().text().catch(()=>void 0)
                }
            });
        }
        let response;
        ...
        if (context.recordEvent) {
            ...
            const body = isStream ? void 0 : await response.clone().text().catch(()=>void 0);
            const httpResponse = {
                attempt,
                status: response.status,
                ok: response.ok,
            ...
            if (!response.ok) context.recordEvent({
                type: 'error',
                ...httpResponse
            });
        }
