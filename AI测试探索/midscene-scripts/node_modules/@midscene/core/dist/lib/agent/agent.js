...
const observation_artifact_namespaceObject = require("@midscene/shared/agent-tools/observation-artifact");
...
const external_file_chooser_js_namespaceObject = require("./file-chooser.js");
const external_insight_js_namespaceObject = require("./insight.js");
...
class Agent {
    ...
    createInsight(getUIContext) {
        return new external_insight_js_namespaceObject.Insight(this.taskExecutor, ()=>this.resolveModelRuntime('insight'), getUIContext);
    }
    ...
    async startObserving(opt) {
        ...
        const observer = new external_ui_observer_js_namespaceObject.UIObserverImpl({
            ...
            createInsight: (record)=>this.createInsight(()=>(0, external_ui_observer_js_namespaceObject.uiContextFromObservationRecord)(record)),
            onStopped: ()=>{
                if (this.activeObserver === observer) this.activeObserver = null;
            },
    ...
    async aiInput(locatePromptOrValue, locatePromptOrOpt, optOrUndefined) {
        ...
        const { locateParam, restParams } = (0, external_yaml_index_js_namespaceObject.buildDetailedLocateParamAndRestParams)(locatePrompt, opt);
    ...
    async aiAct(taskPrompt, opt) {
        ...
        const runAiAct = async ()=>{
            ...
            const { output: actionOutput } = await this.taskExecutor.action(taskPrompt, planningModel, defaultModel, includeLocateInPlanning, aiActContext, cacheable, replanningCycleLimit, imagesIncludeCount, deepThink, void 0, deepLocate, abortSignal, internalReportDisplay);
        ...
        const fileChooserAccepter = this.interface.registerFileChooserListener ? new external_file_chooser_js_namespaceObject.FileChooserAccepter(this.interface) : void 0;
        this.activeFileChooserAccepter = fileChooserAccepter;
        this.activeFileChooserAllowedDir = opt?.fileChooserAllowedDir ? (0, external_node_path_namespaceObject.resolve)(opt.fileChooserAllowedDir) : void 0;
        let aiActError;
        let fileChooserHandlingError;
        let result;
        try {
            if (fileChooserAccept?.length) {
                if (!fileChooserAccepter) throw new Error(`File upload is not supported on ${this.interface.interfaceType}`);
            ...
            this.activeFileChooserAccepter = void 0;
            this.activeFileChooserAllowedDir = void 0;
            try {
        ...
        if (aiActError) throw aiActError.error;
        if (fileChooserHandlingError) throw fileChooserHandlingError;
        return result;
    }
    ...
    async aiQuery(demand, opt) {
        return this.createInsight().aiQuery(demand, opt);
    }
    ...
    async destroy() {
        ...
        this.destroyed = true;
        for (const observer of this.ownedObservers)try {
    ...
    constructor(interfaceInstance, opts){
        ...
        _define_property(this, "ownedObservers", new Set());
        _define_property(this, "executionDumpIndexByRunner", new WeakMap());
        _define_property(this, "fullActionSpace", void 0);
        _define_property(this, "activeFileChooserAccepter", void 0);
        _define_property(this, "activeFileChooserAllowedDir", void 0);
        ...
        const fileChooserActions = this.interface.registerFileChooserListener ? [
            (0, external_device_index_js_namespaceObject.defineActionRegisterFileChooserAccept)(async (files)=>{
                if (!this.activeFileChooserAccepter) throw new Error('RegisterFileChooserAccept can only be used while aiAct is running');
                if (!this.activeFileChooserAllowedDir) throw new Error('RegisterFileChooserAccept requires aiAct option fileChooserAllowedDir');
                await this.activeFileChooserAccepter.registerFromAllowedDir(files, this.activeFileChooserAllowedDir);
            })
        ] : [];
        ...
        Object.defineProperty(this, observation_artifact_namespaceObject.observationArtifactAdapterSymbol, {
            value: {
                exportRecord: async (observation)=>{
                    (0, utils_namespaceObject.assert)(observation instanceof external_ui_observer_js_namespaceObject.UIObservationImpl, 'Cannot export an observation that was not created by this Midscene runtime');
                    return observation.exportRecord();
                },
                loadRecord: (record)=>{
                    (0, external_ui_observer_js_namespaceObject.uiContextFromObservationRecord)(record);
                    return new external_ui_observer_js_namespaceObject.UIObservationImpl(record, this.createInsight(()=>(0, external_ui_observer_js_namespaceObject.uiContextFromObservationRecord)(record)));
                }
            }
        });
    }
