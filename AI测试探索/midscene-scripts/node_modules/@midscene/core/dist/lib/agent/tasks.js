...
__webpack_require__.d(__webpack_exports__, {
    ...
    withFileChooser: ()=>external_file_chooser_js_namespaceObject.withFileChooser
});
...
const multimodal_prompt_js_namespaceObject = require("../ai-model/shared/multimodal-prompt.js");
...
class TaskExecutor {
    ...
    async runPlans(title, plans, planningModel, defaultModel, options) {
        ...
        const { tasks } = await this.convertPlanToExecutable(plans, executionPlanningModel, executionDefaultModel);
    ...
    async action(userPrompt, planningModel, defaultModel, includeLocateInPlanning, aiActContext, cacheable, replanningCycleLimitOverride, imagesIncludeCount, deepThink, fileChooserAccept, deepLocate, abortSignal, reportOptions) {
        return (0, external_file_chooser_js_namespaceObject.withFileChooser)(this.interface, fileChooserAccept, async ()=>this.runAction(userPrompt, planningModel, defaultModel, includeLocateInPlanning, aiActContext, cacheable, replanningCycleLimitOverride, imagesIncludeCount, deepThink, deepLocate, abortSignal, reportOptions));
    }
    ...
    async runAction(userPrompt, planningModel, defaultModel, includeLocateInPlanning, aiActContext, cacheable, replanningCycleLimitOverride, imagesIncludeCount, deepThink, deepLocate, abortSignal, reportOptions) {
        ...
        const referenceImageMessages = await (0, multimodal_prompt_js_namespaceObject.multimodalPromptToChatMessages)((0, external_common_js_namespaceObject.userPromptToMultimodalPrompt)(userPrompt));
        while(true){
            ...
            const result = await session.appendAndRun({
                ...
                executor: async (param, executorContext)=>{
                    ...
                    const planImpl = 'custom' === planningModel.adapter.planning.kind ? planningModel.adapter.planning.planFn : planning_index_js_namespaceObject.standardPlan;
                    let planResult;
                    try {
    ...
    createTypeQueryTask(type, demand, modelRuntime, opt, multimodalPrompt, executionOptions) {
        const queryTask = {
            ...
            executor: async (param, taskContext)=>{
                ...
                if ('Assert' === type) outputResult = Boolean(outputResult);
                return {
                    output: outputResult,
                    log: queryDump,
                    thought
    ...
    async createTypeQueryExecution(type, demand, modelRuntime, opt, multimodalPrompt, executionOptions) {
        ...
        const runner = session.getRunner();
        const executionModelRuntime = {
            ...modelRuntime,
            executionId: runner.id
        };
        const queryTask = await this.createTypeQueryTask(type, demand, executionModelRuntime, opt, multimodalPrompt, executionOptions);
    ...
    async waitFor(assertion, opt, modelRuntime) {
        ...
        while(lastCheckStart - overallStartTime <= timeoutMs){
            ...
            const queryTask = await this.createTypeQueryTask('WaitFor', textPrompt, executionModelRuntime, serviceExtractOpt, multimodalPrompt);
            const result = await session.appendAndRun(queryTask);
