...
__webpack_require__.d(__webpack_exports__, {
    ...
    extractJSONFromCodeBlock: ()=>json_js_namespaceObject.extractJSONFromCodeBlock,
    parseAIObjectResponse: ()=>parseAIObjectResponse,
    callAI: ()=>callAI,
    parseModelResponseJson: ()=>json_js_namespaceObject.parseModelResponseJson,
...
const external_model_call_recorder_js_namespaceObject = require("./model-call-recorder.js");
...
const json_js_namespaceObject = require("../shared/json.js");
...
async function createChatClient({ modelConfig, executionId, recordEvent }) {
    ...
    const openAIOptions = {
        ...
        defaultHeaders: {
            ...
            'x-midscene-version': (0, external_utils_js_namespaceObject.getVersion)(),
...
async function callAI(messages, modelRuntime, options) {
    ...
    const executionId = modelRuntime.executionId ?? `unscoped-${(0, utils_namespaceObject.uuid)()}`;
    const internalCallId = nextInternalCallId();
    const recordEvent = (0, external_model_call_recorder_js_namespaceObject.isModelCallRecordingEnabled)() ? (event)=>{
        (0, external_model_call_recorder_js_namespaceObject.recordModelCallEvent)({
            executionId,
...
async function callAIWithObjectResponse(messages, modelRuntime, options) {
    ...
    return (0, external_semantic_retry_js_namespaceObject.callAiAndParseWithRetry)({
        callAi: (retryAttempt, previousParseError)=>callAI((0, external_semantic_retry_js_namespaceObject.withSemanticRetryFeedback)(messages, previousParseError), modelRuntime, {
                abortSignal: options?.abortSignal,
...
exports.parseAIObjectResponse = __webpack_exports__.parseAIObjectResponse;
