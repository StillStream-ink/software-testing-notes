...
__webpack_require__.d(__webpack_exports__, {
    createEventCounts: ()=>external_common_js_namespaceObject.createEventCounts,
    filterEventsByType: ()=>external_common_js_namespaceObject.filterEventsByType,
    processEventsForLLM: ()=>external_common_js_namespaceObject.processEventsForLLM,
    validateEvents: ()=>external_common_js_namespaceObject.validateEvents,
    generateRecorderYamlTestStream: ()=>generateRecorderYamlTestStream,
    generateRecorderYamlTest: ()=>generateRecorderYamlTest,
    createMessageContent: ()=>external_common_js_namespaceObject.createMessageContent,
    generateYamlTestStream: ()=>generateYamlTestStream,
    getScreenshotsForLLM: ()=>external_common_js_namespaceObject.getScreenshotsForLLM,
    extractInputDescriptions: ()=>external_common_js_namespaceObject.extractInputDescriptions,
    prepareEventSummary: ()=>external_common_js_namespaceObject.prepareEventSummary,
    prepareRecorderGenerationContext: ()=>external_common_js_namespaceObject.prepareRecorderGenerationContext,
...
function createModelRuntime(modelConfig) {
    return (0, index_js_namespaceObject.getModelRuntime)(modelConfig);
}
function createRecorderYamlPrompt(input) {
    const { summary, screenshotAssets } = (0, external_common_js_namespaceObject.prepareRecorderGenerationContext)(input);
    const yamlSummary = {
        ...summary,
        target: input.target,
...
const generateRecorderYamlTest = async (input, modelConfig)=>{
    try {
        const prompt = createRecorderYamlPrompt(input);
        const response = await (0, external_service_caller_index_js_namespaceObject.callAIWithStringResponse)(prompt, createModelRuntime(modelConfig));
...
const generateRecorderYamlTestStream = async (input, options, modelConfig)=>{
    try {
        const prompt = createRecorderYamlPrompt(input);
        const modelRuntime = createModelRuntime(modelConfig);
        if (options.stream && options.onChunk) return await (0, external_service_caller_index_js_namespaceObject.callAI)(prompt, modelRuntime, {
            stream: true,
            onChunk: options.onChunk
        });
        const response = await (0, external_service_caller_index_js_namespaceObject.callAIWithStringResponse)(prompt, modelRuntime);
...
const generateYamlTest = async (events, options, modelConfig)=>generateRecorderYamlTest({
        ...options,
        target: createDefaultWebTarget(events, options),
        events
    }, modelConfig);
const generateYamlTestStream = async (events, options, modelConfig)=>generateRecorderYamlTestStream({
        ...options,
        target: createDefaultWebTarget(events, options),
        events
    }, options, modelConfig);
