...
const errors_js_namespaceObject = require("../ai-model/shared/model-locate-result/errors.js");
const grounding_index_js_namespaceObject = require("../ai-model/workflows/grounding/index.js");
const insight_index_js_namespaceObject = require("../ai-model/workflows/insight/index.js");
...
class Service {
    async locate(query, opt, modelRuntime, abortSignal) {
        ...
        if (!searchArea.config && searchArea.error) {
            ...
            throw new external_types_js_namespaceObject.ServiceError(errorMessage, dump);
        }
        const startTime = Date.now();
        const { parseResult, rect, rawResponse, rawChoiceMessage, usage, reasoning_content } = await (0, grounding_index_js_namespaceObject.AiLocateElement)({
            context,
    ...
    async resolveLocateSearchArea(options) {
        ...
        if (hasPlanLocatedElement) {
            const config = await (0, grounding_index_js_namespaceObject.buildSearchAreaConfig)({
                context,
        ...
        if (adapter.locate.supportsSearchArea) {
            const searchAreaResponse = await (0, grounding_index_js_namespaceObject.AiLocateSection)({
                context,
        ...
        const firstPassLocateResult = await (0, grounding_index_js_namespaceObject.AiLocateElement)({
            context,
            targetElementDescription: queryPrompt,
            modelRuntime,
            abortSignal
    ...
    async extract(dataDemand, modelRuntime, opt, pageDescription, multimodalPrompt, context, executionOptions) {
        ...
        try {
            const result = await (0, insight_index_js_namespaceObject.AiExtractElementInfo)({
                context,
