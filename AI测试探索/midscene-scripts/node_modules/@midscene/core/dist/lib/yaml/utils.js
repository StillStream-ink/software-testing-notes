...
const prompt_context_js_namespaceObject = require("../agent/prompt-context.js");
...
function buildDetailedLocateParam(locatePrompt, opt) {
    ...
    const promptDisplay = 'string' == typeof prompt ? prompt : prompt.prompt;
    const context = opt?.context?.trim() || void 0;
    prompt = (0, prompt_context_js_namespaceObject.buildLocatePromptWithContext)(prompt, opt?.context);
...
function buildDetailedLocateParamAndRestParams(locatePrompt, opt, excludeKeys = []) {
    ...
    if ('object' == typeof opt && null !== opt) {
        ...
        for (const key of allKeys)if (!locateParamKeys.includes(key) && 'context' !== key && 'deepThink' !== key && !multimodalPromptKeys.includes(key) && !excludeKeys.includes(key) && 'locate' !== key) restParams[key] = opt[key];
    }
    return {
        locateParam,
        restParams
