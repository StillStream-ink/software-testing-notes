...
import { type ActionParam, type ActionReturn, type AgentAssertResult, type AgentOpt, type AgentProgressListener, type AgentWaitForOpt, type AssertOptions, type DeepThinkOption, type DeviceAction, ExecutionDump, type InsightAPI, type LocateOption, type LocateResultElement, type OnTaskStartTip, type QueryOptions, type RecordToReportOptions, ReportActionDump, type ScrollParam, type ServiceAction, type ServiceExtractParam, type UIContext } from '../types';
...
import { type UIObserver, type UIObserverOption } from './ui-observer';
...
export declare class Agent<InterfaceType extends AbstractInterface = AbstractInterface> implements InsightAPI {
    ...
    private ownedObservers;
    private get aiActContext();
    private executionDumpIndexByRunner;
    private fullActionSpace;
    private activeFileChooserAccepter?;
    private activeFileChooserAllowedDir?;
    ...
    aiKeyboardPress(locatePrompt: TUserPrompt | undefined, opt: LocateOption & {
        keyName: string;
    }): Promise<void>;
    /**
    ...
    aiQuery<ReturnType = any>(demand: ServiceExtractParam, opt?: QueryOptions): Promise<ReturnType>;
    aiBoolean(prompt: TUserPrompt, opt?: QueryOptions): Promise<boolean>;
    aiNumber(prompt: TUserPrompt, opt?: QueryOptions): Promise<number>;
    aiString(prompt: TUserPrompt, opt?: QueryOptions): Promise<string>;
    aiAsk(prompt: TUserPrompt, opt?: QueryOptions): Promise<string>;
    /**
     ...
     */
    aiLocate(prompt: TUserPrompt, opt?: LocateOption): Promise<Pick<LocateResultElement, "rect" | "center" | "dpr">>;
    aiAssert(assertion: TUserPrompt, msg?: string, opt?: AssertOptions): Promise<AgentAssertResult | undefined>;
