...
function printCommandHelp(scriptName, cmd) {
    const { def } = cmd;
    const positionalUsage = (def.cli?.positionals ?? []).map((name)=>`<${name}>`).join(' ');
    console.log(`\nUsage: ${scriptName} ${cmd.name}${positionalUsage ? ` ${positionalUsage}` : ''} [options]\n`);
    console.log(def.description);
    const globalOptionFlags = new Set(getGlobalOptions().map((option)=>option.flag));
    const schemaEntries = Object.entries(def.schema).filter(([key])=>{
        if (def.cli?.positionals?.includes(key) || def.cli?.options?.[key]?.hidden) return false;
        const { label } = (0, external_cli_args_js_namespaceObject.getCliOptionDisplay)(key, def.cli?.options?.[key]);
    ...
    return [
        {
            ...
            description: 'Print progress while the command is running. act and record print readable progress by default. Use --verbose=jsonl for structured events.'
        },
...
async function runToolsCLI(tools, scriptName, options) {
    ...
    const commands = toolDefinitions.map((def)=>({
    ...
    while(optionStartIndex < restArgs.length && optionStartIndex < positionalNames.length && !restArgs[optionStartIndex].startsWith('-')){
        positionalArgs[positionalNames[optionStartIndex]] = restArgs[optionStartIndex];
        optionStartIndex += 1;
    }
    const parsedArgs = {
        ...positionalArgs,
    ...
    const verboseEnabled = verbose || 'act' === match.name || 'record' === match.name;
