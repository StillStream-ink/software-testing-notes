...
function screenshotBase64FromContext(screenshot) {
    const record = screenshot;
    const base64 = record.base64 ?? record._base64;
    if ('string' == typeof base64) return base64;
    if ('midscene_screenshot_ref' === record.type) throw new Error('StaticPage screenshot is a serialized reference without base64 data');
    throw new Error('StaticPage screenshot must include base64 data before execution');
}
class StaticPage {
    actionSpace() {
