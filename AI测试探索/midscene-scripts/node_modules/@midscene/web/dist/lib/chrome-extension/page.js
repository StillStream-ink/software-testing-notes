...
function waitForTabNavigationComplete(tabId, targetUrl, updatedTab, timeoutMs = NAVIGATION_COMPLETE_TIMEOUT_MS) {
    return new Promise((resolve)=>{
        ...
        const isReady = (tab, navigationUpdateObserved)=>{
            if (!tab || 'complete' !== tab.status) return false;
            const currentUrl = tab.url || tab.pendingUrl || '';
            const isExpectedUrlKind = isBlankUrl(targetUrl) ? isBlankUrl(currentUrl) : !isBlankUrl(currentUrl);
            return isExpectedUrlKind && (navigationUpdateObserved || isTargetTab(tab));
        };
        const onUpdated = (id, info, tab)=>{
            if (id === tabId && isReady(tab, 'complete' === info.status)) finish();
        };
        chrome.tabs.onUpdated.addListener(onUpdated);
        const timer = setTimeout(finish, timeoutMs);
        if (isReady(updatedTab, true)) return void finish();
