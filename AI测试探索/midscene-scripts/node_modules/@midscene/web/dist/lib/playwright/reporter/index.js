...
class MidsceneReporter {
    ...
    finalizeSingleReport(reportFilePath, targetPath) {
        if ((0, external_node_path_namespaceObject.resolve)(reportFilePath) === (0, external_node_path_namespaceObject.resolve)(targetPath)) return;
        ...
        try {
            (0, report_namespaceObject.removeReportArtifact)(reportFilePath);
        } catch (error) {
            (0, utils_namespaceObject.logMsg)(`Error deleting report ${reportFilePath}: ${error}`);
        }
    }
    mergeOwnedReports(tool, targetName) {
