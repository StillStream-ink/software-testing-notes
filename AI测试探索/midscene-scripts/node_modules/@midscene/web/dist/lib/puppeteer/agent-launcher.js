...
async function puppeteerAgentForTarget(target, preference, browser, existingPage) {
    ...
    if ('page' !== mode && 'browser' !== mode) throw new Error(`[midscene] web target mode must be either "page" or "browser", but got "${mode}".`);
    ...
    const agent = 'browser' === mode ? await external_index_js_namespaceObject.PuppeteerBrowserAgent.create(page.browser(), {
        ...commonAgentOpts,
        initialPage: page,
        autoFollowNewPage: runtimeOptions.autoFollowNewPage
    }) : new external_index_js_namespaceObject.PuppeteerAgent(page, {
        ...commonAgentOpts,
