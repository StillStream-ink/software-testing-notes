...
function isTransientNavigationError(error) {
    if (!(error instanceof Error)) return false;
    return /execution context was destroyed|frame was detached|context was destroyed|net::ERR_ABORTED/i.test(error.message);
}
function hasMatchingFrameAspectRatio(frameSize, viewportSize) {
    if (!Number.isFinite(frameSize.width) || !Number.isFinite(frameSize.height) || frameSize.width <= 0 || frameSize.height <= 0) return true;
    const expectedFrameHeight = frameSize.width * viewportSize.height / viewportSize.width;
    return Math.abs(frameSize.height - expectedFrameHeight) <= MAX_FRAME_ASPECT_RATIO_PIXEL_ERROR;
}
...
function jpegSizeFromBase64(data) {
    const bytes = external_node_buffer_namespaceObject.Buffer.from(data.replace(DATA_URL_BASE64_PREFIX, ''), 'base64');
    if (bytes.length < 10 || 0xff !== bytes[0] || 0xd8 !== bytes[1]) return;
    let offset = 2;
    while(offset + 9 <= bytes.length){
        if (0xff !== bytes[offset]) {
            offset++;
            continue;
        }
        while(0xff === bytes[offset])offset++;
        const marker = bytes[offset++];
        if (void 0 === marker || 0x00 === marker || 0xd8 === marker || 0xd9 === marker || marker >= 0xd0 && marker <= 0xd7) continue;
        if (offset + 2 > bytes.length) break;
        const segmentLength = bytes.readUInt16BE(offset);
        if (segmentLength < 2 || offset + segmentLength > bytes.length) break;
        if (isJpegStartOfFrameMarker(marker)) return {
...
class Page {
    ...
    async flushPendingVisualUpdate(force = false) {
        const activeStream = this.activeMjpegStream;
        if (!activeStream || !force && activeStream.hasReceivedScreencastFrame) return;
        try {
            ...
            if (this.activeMjpegStream?.token !== activeStream.token || !force && activeStream.hasReceivedScreencastFrame) return;
            const dataUrl = await this.screenshotBase64();
            if (this.activeMjpegStream?.token !== activeStream.token || !force && activeStream.hasReceivedScreencastFrame) return;
            const frameSize = await (0, img_namespaceObject.imageInfoOfBase64)(dataUrl);
            if (activeStream.expectedViewportSize && !hasMatchingFrameAspectRatio(frameSize, activeStream.expectedViewportSize)) throw new Error(`Screenshot fallback aspect ratio mismatch: expected viewport ${activeStream.expectedViewportSize.width}x${activeStream.expectedViewportSize.height}, received ${frameSize.width}x${frameSize.height}`);
            debugPage('screencast fallback screenshot size %dx%d for viewport %dx%d', frameSize.width, frameSize.height, activeStream.expectedViewportSize?.width ?? 0, activeStream.expectedViewportSize?.height ?? 0);
            ...
            if (isTransientNavigationError(error) && !isClosedPageError(error)) return;
            activeStream.onError?.(error);
        }
    }
    queuePendingVisualUpdate(force = false) {
    ...
    schedulePendingVisualUpdate(force = false) {
        if (!this.activeMjpegStream) return;
        this.queuePendingVisualUpdate(force);
        if (this.visualUpdateFollowupTimer) clearTimeout(this.visualUpdateFollowupTimer);
        this.visualUpdateFollowupTimer = setTimeout(()=>{
            this.visualUpdateFollowupTimer = void 0;
            this.queuePendingVisualUpdate(force);
    ...
    async startMjpegStream(options) {
        ...
        const handleFrame = (event)=>{
            (async ()=>{
                ...
                const isExpectedFrame = !expectedViewportSize || hasMatchingFrameAspectRatio(frameSize, expectedViewportSize);
                if (!hasLoggedScreencastFrameSize && expectedViewportSize) {
                    hasLoggedScreencastFrameSize = true;
                    debugPage('CDP screencast JPEG size %dx%d (metadata %dx%d) for viewport %dx%d', frameSize.width, frameSize.height, compositorFrameSize.width, compositorFrameSize.height, expectedViewportSize.width, expectedViewportSize.height);
                }
                if (isExpectedFrame) {
                    if (this.activeMjpegStream?.token === streamToken) this.activeMjpegStream.hasReceivedScreencastFrame = true;
                    try {
                        onFrame({
                            data: event.data,
                            contentType: 'image/jpeg'
                ...
                } else if (!hasReportedInvalidScreencastFrame) {
                    hasReportedInvalidScreencastFrame = true;
                    debugPage('CDP screencast frame aspect ratio mismatch: expected viewport %dx%d, received %dx%d; using screenshot fallback until the screencast recovers', expectedViewportSize?.width ?? 0, expectedViewportSize?.height ?? 0, frameSize.width, frameSize.height);
                    if (this.activeMjpegStream?.token === streamToken) this.activeMjpegStream.hasReceivedScreencastFrame = false;
        ...
        try {
            ...
            try {
                ...
                if (this.activeMjpegStream?.token === streamToken) this.activeMjpegStream.expectedViewportSize = expectedViewportSize;
    ...
    async openFrameSource() {
        ...
        return {
            latest: ()=>latest,
            decode: async (refs)=>refs.map((frameRef)=>frameRef.ref),
            stop: ()=>handle.stop()
        };
    }
    async url() {
    ...
    get keyboard() {
        return {
            type: async (text, options)=>{
                ...
                debugPage(`keyboard type ${text}${void 0 !== effectiveDelay ? ` (delay: ${effectiveDelay}ms)` : ''}`);
    ...
    constructor(underlyingPage, interfaceType, opts){
        ...
        _define_property(this, "visualUpdateForceQueued", false);
        _define_property(this, "visualUpdateFollowupTimer", void 0);
